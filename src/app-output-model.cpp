...
OutputModel::OutputModel(int depth, bool msb_first, RasterizerFunc rasterizer_func, std::string_view cmd_line):
...
void OutputModel::add_glyph(const app::Glyph& glyph) {
    font2c_glyph_t f2c_glyph = {
            ...
            .bitmap = reinterpret_cast<const uint8_t*>(m_pixel_data.size()),
...
void OutputModel::write(std::string_view path, std::string_view font_path, const app::Options& options) const {
    ...
    fmt::print(f, " * Pseudo Bold:          {}\n", options.pseudo_bold ? "yes" : "no");
    ...
    for (const auto& glyph: m_glyphs) {
        fmt::print(f, "    {{0x{:08X}, &PIXELS[0x{:08X}], {:>6}, {:>6}, {:>6}, {:>6}, {:>6}}},\n",
                   glyph.codepoint, reinterpret_cast<std::uintptr_t>(glyph.bitmap), glyph.x_bearing, glyph.y_bearing,
                   glyph.width, glyph.height, glyph.x_advance + options.kerning_offset);
    }
...
static std::string snake_to_pascal(const std::string& s) {
    std::string result;
    result.reserve(s.size());
    bool upper_next = true; // capitalise first letter
...
void OutputModel::write_java_metadata(std::string_view path, std::string_view font_path,
        const app::Options& options) const {
    const File f(path);
    const std::string class_name = std::filesystem::path(path).stem().string();
    ...
    fmt::print(f, "public class {} {{\n", class_name);
    fmt::print(f, "    public static final Map<Integer, Glyph> GLYPHS = Map.ofEntries(\n");

    auto glyph_i = m_glyphs.begin();
    const auto glyph_e = m_glyphs.end();

    while (glyph_i < glyph_e) {
        const auto next = glyph_i + 1;
